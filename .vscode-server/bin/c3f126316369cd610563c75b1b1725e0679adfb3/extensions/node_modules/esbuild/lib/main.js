...
function createChannel(streamIn) {
  ...
  let buildOrServe = (args) => {
    ...
    if (plugins && plugins.length > 0) {
      ...
      handlePlugins(options, plugins, key, details).then(([requestPlugins, pluginRefs]) => buildOrServeContinue(__objSpread(__objSpread({}, args), {
        key,
        details,
        requestPlugins,
        pluginRefs
...
var ensureServiceIsRunning = () => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.11.12"}`, "--ping"), {
    windowsHide: true,
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.11.12"}`), {
    cwd: defaultWD,
    windowsHide: true,
    input: stdin,
